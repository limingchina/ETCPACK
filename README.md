...
Only RGB format is supported now. The compression is using the code path `compressBlockDiffFlipFast`. Inside this implementation, in the C++ code, there are two code paths: `compressBlockDiffFlipAverage` and `compressBlockDiffFlipCombined`. The selection logic is not implemented yet. Currently `compressBlockDiffFlipAverage` is used, though the `compressBlockDiffFlipCombined` is also working.
